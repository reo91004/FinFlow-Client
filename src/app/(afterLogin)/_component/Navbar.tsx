...
export default function Navbar() {
    ...
    // 통화/프로필/포트폴리오 드롭다운 열림 여부 state
    const [currencyOpen, setCurrencyOpen] = useState(false);
    const [profileOpen, setProfileOpen] = useState(false);
    const [portfolioOpen, setPortfolioOpen] = useState(false);
    const [portfolioMenuOpen, setPortfolioMenuOpen] = useState(false);

    // 예시 포트폴리오 목록과 선택된 포트폴리오 state
    ...
    // pathname이 변경될 때마다 portfolioMenuOpen을 false로 설정
    useEffect(() => {
    ...
    const handleSelectPortfolio = (portfolio: string) => {
        ...
        setPortfolioMenuOpen(false);
        // 필요한 추가 로직(페이지 이동, 상태관리 등)이 있다면 여기에 작성
    };
                        ...
                        <div className="absolute right-0 mt-1 w-52 rounded-[0.5rem] bg-white shadow-xl">
                            <div className="py-1" role="menu">
                                <div className="block w-full px-4 py-2 flex gap-2 items-center border-b">
                                    <Image src={Profile} className="rounded-md" width="32" alt="프로필 이미지" />
                                    <span className="text-slate-500 font-semibold truncate">테스트</span>
                                </div>
                                <Link
                                    href="/user-profile"
